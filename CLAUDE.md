...
- **Alcor OMEA 8C** (`alcor.py`). RGB FITS. `load_alcor_fits()` returns a zenith-centered, north-up RGB array plus an ARC-projection WCS that maps pixel ↔ (azimuth, altitude) with altitude=0 at `horizon_radius` pixels from zenith. The `load_alcor_fits` ARC WCS is refined by a fitted odd-power radial (k3) SIP distortion plus a recentered zenith and rotation, calibrated by `fit_alcor_wcs()` against `bright_star_sloan.fits` (Vmag <= 4) over dark-sky frames (Sun < -18 deg); the camera's `DATE` header is the true UT (`DATE-OBS` is local time despite its label). The fitted geometry (center shift, rotation, radial k3) lives in a time-indexed `ALCOR_CALIBRATIONS` table; `load_alcor_fits` resolves the epoch nearest a frame's time (filename timestamp, DATE-header fallback) when its calibration kwargs are left unset, so frames from different eras (e.g. 2024 vs 2026) each get the right geometry, and explicit kwargs override resolution. `plot_alcor_fits()` applies the empirically-tuned per-channel scale factors (`gscale`/`bscale`) and a power+ZScale stretch for display; `alcor_proc_fits()` writes the processed cube + WCS as a FITS file (data is `np.flipud`'d into FITS lower-origin convention so DS9 displays north-up while the WCS still resolves correctly). The matcher (`assign_alcor_matches`) is seeded by the resolved epoch geometry and uses a kd-tree with local-asterism pattern verification plus a local relative-brightness tie-break (cloud extinction is patchy, so only relative flux among nearby contested stars is trusted) rather than per-frame nearest-neighbor refitting.
...
alcor_proc_fits <input.fits> [-o OUT] [--overwrite] [--rotation ...] [--xcen ...] ...
#   Writes a zenith-centered, north-up FITS cube with alt/az WCS.
#   Default output: <input>_proc.fits.

plot_alcor_fits <input.fits> [-o OUT.pdf] [--outimage RAW] [--gscale ...] ...
