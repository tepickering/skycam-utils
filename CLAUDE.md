...
- **Alcor OMEA 8C** (`alcor.py`). RGB FITS. `load_alcor_fits()` returns a zenith-centered, north-up RGB array plus an ARC-projection WCS that maps pixel ↔ (azimuth, altitude) with altitude=0 at `horizon_radius` pixels from zenith. `plot_alcor_fits()` applies the empirically-tuned per-channel scale factors (`gscale`/`bscale`) and a power+ZScale stretch for display; `alcor_proc_fits()` writes the processed cube + WCS as a FITS file (data is `np.flipud`'d into FITS lower-origin convention so DS9 displays north-up while the WCS still resolves correctly).
