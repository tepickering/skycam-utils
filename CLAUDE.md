...
Cloud-extinction maps (`alcor/extinction.py`): every frame already measures `ext_*` = (calibrated − catalog) for a few hundred bright stars at known (az, alt) — a sparse, irregular sampling of line-of-sight extinction over the dome. This module turns that into a map. **The block size is 10 frames (~5 min), and that number is measured, not assumed**: on a clear moon-free night the robust scatter of block means σ(N) tracks the uncorrelated-noise line σ(1)/√N to within 5% out to N=12 and has clearly departed by N=30, bottoming near a ~0.02 mag per-star floor — which is right where the ~0.03 mag epoch-to-epoch zeropoint stability sits, so averaging longer cannot help. 10 frames balances that against update cadence (`claude_docs/scripts/frame_averaging_floor.py` is the analysis). Stars are restricted to `ALCOR_EXT_MAG_WINDOW` (−11.0 to −9.5 instrumental), the range free of BOTH the bright-star CMOS non-linearity and the faint-end bias; variables are excluded because a variable has no catalog magnitude so its `ext_*` is NaN by construction. **Smoothing runs on a 1° alt/az grid and is then resampled to the raw frame, never directly onto raw pixels** — that is a hard performance requirement, not tidiness: 2M pixels × ~350 stars is ~700M great-circle distances per map, hopeless at a 5-minute cadence, while the same 8° kernel on a 1° grid is ~11M and takes milliseconds (and an 8° kernel is not resolved better by a finer grid, so nothing is lost). The grid is NaN where the summed kernel weight is below `ALCOR_EXT_MIN_WEIGHT`; **blanking rather than extrapolating is the point**, because an unsampled region is precisely the one most likely to be the most extinguished. For the same reason **non-detections are tracked separately as lower limits**: a star behind thick cloud has `flux=0` → `mag` NaN → `ext` NaN, so it would silently vanish exactly where extinction is highest — instead it lands in the FITS `STARS` table with `detected=False` and is drawn as an open marker. The output is the raw `(ny, nx)` frame with the alt/az ARC WCS, matching `alcor_sky_brightness_fits`/`alcor_proc_fits`, so maps overlay pixel-for-pixel on every other product and `alcor_extinction_at(source, az, alt, radius=1.0)` resolves a pointing in one `world_to_pixel` call — **`radius` is in DEGREES** (great-circle), and takes a median over that cone; since the map is already 8°-smoothed the cone adds almost no smoothing, its job is surviving a blank pixel, a resampling edge, or a position just outside the valid region. `PIXSCALE` in the header (radial px/deg; the ARC projection is equidistant in radius so one number is exact) makes that lookup cheap — no per-pixel WCS transform needed. `EXTFLAT='none'` reserves the slot for a clear-night flat field: **there is a real residual altitude trend** (measured on 2025-01-01: ≈ −0.12 mag at alt 20–30°, ≈ 0 at 45–60°, ≈ −0.09 at 75–90°), recorded but deliberately NOT corrected until several clear nights are in hand — one night is a thin basis for a correction applied to every map forever, and it could mask a real low-altitude dome. Validated against three graded nights (2025-01-01 clear, 01-02 light cloud in the second half, 01-03 cloud all night, per the observer): the clear night reads −0.02 to −0.07 mag flat across the dome all night, which is the null result and sets the systematic floor. Operationally the maps are built by cron every 5 minutes from the newest `*_phot.csv` the real-time ingest has written; `scripts/mmt_extinction_redis.py` (NOT packaged — `redis` stays out of the package's dependencies, matching `mmt_position.py`/`tcs_logger.py`) then looks up the MMT's pointing and publishes `allsky_extinction*`. Note a raw-frame map is ~8 MB, so the cron should overwrite a single rolling `alcor_extinction_latest.fits` rather than keep every map (~2.3 GB/day); the night's history lives in the animation, and if a queryable time series is ever wanted the 1° alt/az grid is the thing to archive (131 KB). Tested in `test_alcor_extinction.py`.
...
alcor_extinction_map <inputs...> [-o OUT.fits] [--plot OUT.png] [--latest N] [--nframes 10] [--band g] [--method ap] [--kernel-sigma 8] [--min-altitude 20] [--vmax 1.5] [--max-age S] [--at AZ ALT] [--radius 1.0] [--overwrite]
#   Builds ONE cloud-extinction map from per-frame *_phot.csv photometry (files
#   or a directory). Every frame measures ext_g = calibrated - catalog for a few
#   hundred stars at known (az, alt): a sparse sampling of line-of-sight
#   extinction. Averages a block of frames, kernel-smooths onto the sky, writes
#   the raw-frame float32 map + alt/az ARC WCS (same geometry as
#   alcor_sky_brightness_fits / alcor_proc_fits, so it overlays pixel-for-pixel
#   and a pointing is one world_to_pixel call away), plus a STARS bintable of
#   every contributing star. --latest N uses only the N newest frames: the
#   real-time path. --at prints the value at one position; --max-age exits
#   NON-ZERO when the newest frame is older than S seconds, so a cron alerts
#   instead of publishing a stale map that looks exactly like a current one.

plot_alcor_extinction_fits <map.fits> [-o OUT.png] [--vmax 1.5] [--cmap inferno_r] [--dpi 140]
#   Re-renders a saved map from its STARS table without recomputing it.

alcor_extinction_movie <inputs...> [-o OUT.mp4] [--nframes 10] [--stride N] [--fps 12] [--band g] [--method ap] [--vmax 1.5] [--cmap inferno_r] [--frames-dir DIR] [--keep-frames] [--title T] [--quiet]
#   Animates a night of maps. --stride defaults to --nframes (independent,
#   non-overlapping averages); a smaller stride overlaps successive windows,
#   which looks smoother but CORRELATES adjacent animation frames -- worth
#   knowing before reading structure off the movie. Frames render to a temp dir
#   and are encoded with ffmpeg (the pad filter is required: H.264 needs even
#   dimensions and matplotlib emits odd ones).
...
`alcor` is a **package**, split by concern out of what was a single 5700-line `alcor.py`: `config` (constants, `ALCOR_CALIBRATIONS`, `ALCOR_ZEROPOINTS`), `timeutils` (frame time, Sun/Moon, dark-frame selection), `wcs` (ARC WCS + distortion model), `wcsfit` (detection, matching, `fit_alcor_wcs`), `masks` (loading the badpix/horizon assets), `badpix`, `horizon`, `catalogs`, `io` (`load_alcor_fits`, `_corner_bias`), `photometry`, `display`, `skybright`, `extinction` (cloud-extinction maps), `keogram`, `night` (`alcor_process_night`), `ledger` (the archive run ledger), `archive` (`alcor_process_archive`), and `cli` (all 19 entry points). **`alcor/__init__.py` re-exports the entire namespace**, private names included, so `from skycam_utils.alcor import <anything>` and the `skycam_utils.alcor:*_cli` entry points work exactly as before — nothing outside the package had to change. Dependencies run one way (`config → timeutils → wcs → masks → {badpix, horizon, catalogs} → io → {photometry, display, skybright} → extinction → keogram → night → archive → cli`, with `ledger` a dependency-free leaf that only `archive` imports); the two cycles that a naive split creates are worth knowing about, since both are easy to reintroduce: `badpix` needs the horizon mask while `horizon` needs the badpix mask (broken by keeping both *loaders* in `masks`, separate from the *builders*), and `photometry` needs the check plot while `display` needs `_corner_bias` (broken by `_corner_bias` living in `io`, where a frame-level bias estimate belongs). `claude_docs/scripts/split_alcor.py` is the record of how the split was done.
